...
const titleFrontPage = {
  font_face: 'Times New Roman',
  font_size: 12,
  bold: true
}
const titleFrontPageInfo = {
  font_face: 'Times New Roman',
  font_size: 12
}
const pNumTitle = {
...
const checkBox = String.fromCharCode( 9723 );

const footerP = docx.getFooter().createP();
...
let tenant2, tenant3, address, unitNum, city, zip, createdDate, checkFamResidence, checkMultiResidence, unitType, county, hoa, hoaDocs, hoaname, noPetCheck, petCheck, noKnowlLead,KnowlLead,numMail,mailBoxNum,numResidence,numOpener,numGate,commonArea,parkPerm,numStorSpace,numParkSpot,storNum,parkNum, leadDisclosuresApply,startRent,endRent, petName1, petName2, petType1, petType2, petDescription1, petDescription2,propOwner,date,securityDeposit,baseRent,petRent,parkingRent,storageRent,applianceRent,otherRent,totalRent,guarantors;

let applicant, residents, resident1, resident2, resident3, guarantor, dependents, dependent1, dependent2,dependent3, dependent4 ;

// address = '3668 Quimby St'; unitNum = 1; city = 'San Diego'; zip = 92106;

noPetCheck = String.fromCharCode( 9724 );
...
async function wordGen(information) {

  // tenant1 = `${information[0].applicant.first} ${information[0].applicant.last}`; 
  // tenant2 = `${information[0].addApplicant.first} ${information[0].addApplicant.last}`; 
  // tenant3 = `${information[0].dependents.first} ${information[0].dependents.last}`;
  applicant = ' ';
  tenant2 = ' ';
  tenant3 = ' ';
  tenant4 = ` `;
  ...
  /*If else for if it has a unit num */
  unitNum = information[0]["unitNumber"]; 
  city = information[1]["City"];
  zip = information[1]["Zip Code"];
  county = information[1]["County"]; //Grab information
  unitType = information[1]["Property Type"]; // Grab information
  numMail = ''; //Grab information number of mailbox
  mailBoxNum = information[1]["Mailbox #"]; //mailbox number
  numResidence = information[1]["Number of House Keys"]; //residence number
  numOpener = information[1]["Number of Garage Remotes"]; //garage opener number if there is one
  numGate = information[1]["Number of Gate Remotes"]; //number for garage gate if there is one
  commonArea = information[1]["Number of Community Keys"]; //yes or no for common area
  parkPerm = information[1]["Number of Parking Permits"]; //yes or no for park permit
  numStorSpace = information[1]["Number of Storage Spaces"]; // number of storage space if any
  storNum = information[1]["Storage Area Number"]; //number of storage space if any
  numParkSpot = ''; // number of parking spot if any
  parkNum = ''; //number of parking spot if any
  baseRent = information[1]["Base Rent"]; //number
  petRent = information[1]["Pet Rent"];
  parkingRent = information[1]["Parking Rent"];
  storageRent = information[1]["Storage Rent"];
  applianceRent = information[1]["Appliance Rent"];
  otherRent = information[1]["Other Rent"];
  totalRent = information[1]["Rent"];
  securityDeposit = information[1]["Security Deposit"];
  startRent = information[1]["Start Rent Date"];
  endRent = information[1]["End Rent Date"];
  propOwner = information[1]["Property Owner Text"];
  elecCustRecord = information[1]["ElectricityCustomerOfRecord"];
  gasCustRecord = information[1]["GasCustomerOfRecord"];
  watCustRecord = information[1]["WaterCustomerOfRecord"];
  elecChargedRes = information[1]["ElectricityChargedToResident"];
  gasChargedRes = information[1]["GasChargedToResident"];
  watChargedRes = information[1]["WaterChargedToResident"];
  calcMethodElec = information[1]["CalculationMethodElectricity"];
  calcMethodGas = information[1]["CalculationMethodGas"];
  calcMethodWat = information[1]["CalculationMethodWater"];
  sewerCustRecord = information[1]["SewerCustomerRecord"];
  sewerChargedRes = information[1]["SewerChargedToResident"];
  calcMethodSew = information[1]["CalculatonMethodSewer"];
  commonAreaWat = information[1]["CommonAreaWater"];
  commonAreaSew = information[1]["CommonAreaSewer"];
  trashCustRecord = information[1]["TrashCustomerOfRecord"];
  trashChargedRes =information[1]["TrashChargedToResident"];
  calcMethTrash = information[1]["CalculationMethodTrash"];
  commonAreaTrash = information[1]["CommonAreaTrash"];
  cableCustRecord = information[1]["CableCustomerOfRecord"];
  cableChargedRes = information[1]["CableChargedToResident"];
  calcMethodCable = information[1]["CalculationMethodCable"];

  let newStartRent = startRent;
  let newEndRent = endRent;

  createdDate = today;

  hoaname = information[1]["HOA Name"];
  hoa = String.fromCharCode( 9724 );
  hoaDocs = String.fromCharCode( 9724 );
  noKnowlLead = String.fromCharCode( 9724 );
  KnowlLead =  String.fromCharCode( 9723 );

  if(unitType === "House"){
    //filled
    checkFamResidence = String.fromCharCode( 9724 );
    checkMultiResidence = String.fromCharCode( 9723 );
  }else{
    // Not filled
    checkFamResidence = String.fromCharCode( 9723 );
    checkMultiResidence = String.fromCharCode( 9724 );
  }
  // if(){
  //   //filled
    noPetCheck = String.fromCharCode( 9724 );
    //   petCheck = String.fromCharCode( 9723 );
  // }else{
  //   // Not filled
  //   noPetCheck = String.fromCharCode( 9724 );
  //   petCheck = String.fromCharCode( 9723 );
  ...
  // }else{
  //   // Not filled
  //   petCheck = String.fromCharCode( 9724 );
  // }
   if(hoaname !== 'None'){   
    hoa = String.fromCharCode( 9724 );
   }else{
     hoa = String.fromCharCode( 9723 );
   }
  if(information[1]["year built"] <= 1978){   
    leadDisclosuresApply = String.fromCharCode( 9724 );
  }else{
    leadDisclosuresApply = String.fromCharCode( 9723 );
  }

  applicant = `${information[0].applicant}`;

  // console.log(information[0].pets.length);
  // console.log(JSON.stringify(information[0].pets));
  // console.log('===============');
  // console.log(information[0].residents.length);
  // console.log(information[0].dependents.length);
  // console.log(`===============`);
  // console.log(JSON.stringify(information[0].dependents[0]));
  // console.log(`===============`);
  // console.log(JSON.stringify(information[0].residents));
  // console.log(`===============`);
  ...
  let residentObj = isObject(information[0].residents);
  let dependentObj = isObject(information[0]['dependents']);



  if(information[0]['residents'] === null){
    resident1 = '';
    resident2 = '';
    resident3 = '';
  } 
  if(information[0].residents != null && information[0].residents.length === 1){
      resident1 = `${information[0].residents[0].key_0} ${information[0].residents[0].key_1} ${information[0].residents[0].key_2}`;
      resident2 = '';
      resident3 = '';
      console.log(resident1)
  }
  if(information[0].residents != null && information[0].residents.length === 2) {
      resident1 = `${information[0].residents[0].key_0} ${information[0].residents[0].key_1} ${information[0].residents[0].key_2}`;
      resident2 = `${information[0].residents[1].key_0} ${information[0].residents[1].key_1} ${information[0].residents[0].key_2}`;
      
    resident3 = '';
      console.log(resident1)
      console.log(resident2)
  }
  if(information[0].residents != null && information[0].residents.length === 3){
      resident1 = `${information[0].residents[0].key_0} ${information[0].residents[0].key_1} ${information[0].residents[0].key_2}`;
      resident2 = `${information[0].residents[1].key_0} ${information[0].residents[1].key_1} ${information[0].residents[0].key_2}`;
      resident3 = `${information[0].residents[2].key_0} ${information[0].residents[2].key_1} ${information[0].residents[0].key_2}`;
      console.log(JSON.stringify(information[0].residents));
      console.log('----------------------')
      console.log(JSON.stringify(information[0].residents[0]));
      console.log(resident1)
      console.log(resident2)
  ...
  if(information[0].dependents == null){
    dependent1 = '';
    dependent2 = '';
    dependent3 = '';
    dependent4 = '';
  }    
  if(information[0].dependents != null && information[0].dependents.length === 1){
      dependent1 = `${information[0].dependents[0].key_0} ${information[0].dependents[0].key_1}`;
      console.log(dependent1)
  }
  if(information[0].dependents != null && information[0].dependents.length === 2) {
      dependent1 = `${information[0].dependents[0].key_0} ${information[0].dependents[0].key_1}`;
      dependent2 = `${information[0].dependents[1].key_0} ${information[0].dependents[1].key_1}`;
      console.log(dependent1)
      console.log(dependent2)
  }
  if(information[0].dependents != null && information[0].dependents.length === 3){
      dependent1 = `${information[0].dependents[0].key_0} ${information[0].dependents[0].key_1}`;
      dependent2 = `${information[0].dependents[1].key_0} ${information[0].dependents[1].key_1}`;
      dependent3 = `${information[0].dependents[2].key_0} ${information[0].dependents[2].key_1}`;
      console.log(dependent1)
      console.log(dependent2)
      console.log(dependent3)
  }
  if(information[0].dependents != null && information[0].dependents.length === 4){
      dependent1 = `${information[0].dependents[0].key_0} ${information[0].dependents[0].key_1}`;
      dependent2 = `${information[0].dependents[1].key_0} ${information[0].dependents[1].key_1}`;
      dependent3 = `${information[0].dependents[2].key_0} ${information[0].dependents[2].key_1}`;
      dependent4 = `${information[0].dependents[3].key_0} ${information[0].dependents[3].key_1}`;
      console.log(dependent1)
      console.log(dependent2)
      console.log(dependent3)
      console.log(dependent4)
  }

  if(dependentObj == 1) {
      dependent1 = `${information[0].dependents[0].key_0} ${information[0].dependents[0].key_1} ${information[0].dependents[0].key_2}`;
      console.log('[Object Dependent]', + dependent1);
  }
  
  if(residentObj == true){
      resident1 = `${information[0].residents.key_0} ${information[0].residents.key_1} ${information[0].residents.key_2}`
  
      console.log(`[Object Resident] ${resident1}`);
  }

  if(information[0].guarantors == null){
    guarantor = '';
  } else {
    guarantor = `${information[0].guarantors}`
  }

  if(information[0].pets.length == null){
    petName1 = '';
    petName2 = '';
    petType1 = '';
    petType2 = '';
  ...
  if(information[0].pets[0].key_0 == 'Applicant did not provide any pets.'){
    petName1 = '';
    petName2 = '';
    petType1 = '';
    petType2 = '';
  ...
  if(information[0].pets.length == 1){
    petName1 = information[0].pets[0].key_0;
    petName2 = '';
    petType1 = information[0].pets[0].key_1;
    petType2 = '';
    petDescription1 = '';
    petDescription2 = '';

  }
  if(information[0].pets.length == 2){
    petName1 = information[0].pets[0].key_0;
    petName2 = information[0].pets[1].key_0;
    petType1 = information[0].pets[0].key_1;
    petType2 = information[0].pets[1].key_1;
    petDescription1 = '';
    petDescription2 = '';
  }

  if(information[0].pets[0].key_1 == `undefined`){
    petType1 = ' '
  }


  console.log('----------------------')
  console.log(`[resident1] ${resident1}`);
  console.log(`[dependent1] ${dependent1}`);
  // console.log(`[dependent2] ${dependent2}`);
  // console.log(`[dependent3] ${dependent3}`);
  // console.log(`[dependent4] ${dependent4}`);
  ...
  var termTable = [
    ...
    [`Start Rent Date`,`${newStartRent}`],
    [`End Rent Date`,`${newEndRent}`],
  ]
  ...
  var rentTable = [
    ...
    [`Monthly Appliance Rent`,` ${applianceRent}`],
  ...
  var utilitiesTable = [
    ...
    [`Gas`,`${gasCustRecord}`,`${gasChargedRes}`,`${calcMethodGas}`,``],
    [`Electricity`,`${elecCustRecord}`,`${elecChargedRes}`,`${calcMethodElec}`,``],
    [`Water`,`${watCustRecord}`,`${watChargedRes}`,`${calcMethodWat}`,`${commonAreaWat}`],
    [`Sewer`,`${sewerCustRecord}`,`${sewerChargedRes}`,`${calcMethodSew}`,`${commonAreaSew}`],
    [`Trash`,`${trashCustRecord}`,`${trashChargedRes}`,`${calcMethTrash}`,`${commonAreaTrash}`],
    [`Cable`,`${cableCustRecord}`,`${cableChargedRes}`,`${calcMethodCable}`,``],
    [`Landscape Maintenance`,``,``,``,``],
    [`Landscape Watering`,``,``,``,``]
  ]

  var initialAmountsDue = [
    [{
      val:"Category",
      opts: {
          cellColWidth: 2000,
    ...
    {
      val:"Payment Received to Date by Landlord",
      opts: {
          cellColWidth: 1200,
          align: "center",
          shd: {
    ...
    [`Security Deposit`,`$ `,`$ `,`$ `,``],
    [`Base Rent from       (date) through       (date)
    If any concessions have been granted to Resident for any portion of this time period, the amount above is the Base Rent due after application of the concession amount. `,`$ `,`$ `,`$ `,``],
    [`Other   (describe)`,`$ `,`$ `,`$ `,``],
    [`Other   (describe)`,`$ `,`$ `,`$ `,``],
    [`Other   (describe)`,`$ `,`$ `,`$ `,``],
    [`Total`,`$`,`$`,`$`],
  ...
  var tableStyle = {
    ...
    borderColor: "#e6eaeb", // color for border
    borderSize: "8", // size of border width
    fontSize: 8
  }
  ...
  const data = [
    ...
    [
      {
      type: "image",
      path: path.resolve(__dirname, './House_applications/logo.png')
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {type:'linebreak'},
    ...
    [
      {
        type:'text',
        val:'                                                                                           Kimball, Tirey & St. John California Residential Lease/Rental Agreement     ',
        opt:footer
      },
      {type:'linebreak'},
      {
        type:'text',
        val:'                                                                                                   Ó 2003-2018 Kimball, Tirey & St. John LLP.  All rights reserved.  ',
        opt:footer
      },
      {type:'linebreak'},
      {
        type:'text',
        val:'                                                             This lease may not be duplicated in any way without the express written consent of Kimball, Tirey & St. John LLP.',
        opt:footer
      },
      {type:'linebreak'},
      {
        type:'text',
        val:'                                                                                     Licensed for use on properties owned or managed by HomeTean Property Management',
        opt:footer
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:"                                                                                                       Landlord's Initials ___",
        opt: footer
      },
      {
        type:'text',
        val:"              Resident's Initials ___ ___ ___ ___",
        opt: footer
      },
      {type:'linebreak'},
      {
    ...
    [
      ...
      {
        type:'text',
        val:`${checkMultiResidence} (If filled) Part of a multi-family residential complex`,
        opt:titleFrontPageInfo
      },
      {type:'linebreak'},
      {type:'linebreak'},
    ...
    [
      ...
      {
        type:'text',
        val:`(Applied if payments have not been received within 3 days of their due date):`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
    ...
    [
      {
        type:'text',
        val:`Landlord and Resident agree that it is impractical or extremely difficult to determine the exact amount of the utilities consumed by Resident (and/or in the common areas), but that the methods used to determine Resident’s share described above are reasonably accurate estimates.  Resident acknowledges that under the billing method described above Resident may be paying for trash/recycling, water and sewer usage in the common areas or in other residential units.  Resident further acknowledges and understands that the amount of the monthly bill will fluctuate, depending on actual usage and actual billings from the utility providers.`,
        opt: p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`If formula methods are used to calculate Resident’s share for water, all water related charges assessed to Landlord may be used to calculate the amount charged to Resident. These may include (but are not limited to) water related charges contained on tax bills, and all charges contained on the local water provider’s bills to Landlord.`,
        opt: p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      ...
      {
        type:'text',
        val:`“Water” charges include charges for gas or electricity used to heat hot water in addition to the cost of the water.`,
        opt:p1Options
      },
      {type:'linebreak'},
    ],
    [
      {
        type:'text',
        val:`If either submetering or formula methods are used to calculate Resident’s share for sewer, all sewer related charges assessed to Landlord may be used to calculate the amount charged to Resident. These may include (but not limited to) sewer related charges contained on tax bills, and all charges contained on the local sewer provider’s bills to Landlord.`,
        opt: p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`If Resident is responsible for trash/recycling charges, all trash and recycling related charges assessed to the Landlord may be used to calculate the amount charged to each Resident. These may include (but are not limited to), trash/recycling related charges contained on tax bills, and all charges contained on utility bills received from the local trash/recycling providers.`,
        opt: p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`Submetered Utilities: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`Billings based on submeter readings (if any) will itemize the beginning and ending meter readings, the rate charged to Resident, and all categories of  information that appear within the utility’s standard billing format to the Landlord.   Billing amounts based on submeter readings will be determined by:`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`         ${checkBox} Dividing the total fixed charges equally among the total number of submetered consumers which do not receive a separate master bill, plus adding Resident’s share (determined from the submeter readings) of the variable charges at the same rate charged in the master bill.  (Use this option if the Property is located in the City of San Diego, and water is submetered).`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`         ${checkBox} Dividing the total master bill according to submeter readings.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`If the Premises are submetered, Resident agrees to allow Landlord, or a billing service provider Landlord designates, access to the Premises to install, repair, remove and read submeters. If the unit is located in the City of San Diego, all submeter billing will be in compliance with Chapter 6, Article 7, Division 6, Sections 67.0601 through 67.0608 of the San Diego Municipal Code.`,
        opt: p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      ...
      {
        type:'text',
        val:`Formula Billing Used: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`Utilities billed to the Property are allocated between all Property residents as indicated above. Resident’s percentage is calculated based on:`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      ...
      {
        type:'text',
        val:`The approximate square footage of Resident’s unit as compared to the total square footage of all occupied units within the 	Property.  Resident’s share is       %  if all units are occupied.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      ...
      {
        type:'text',
        val:` Authorized occupant method: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`The number of authorized occupants residing in Resident’s unit as compared to all authorized occupants in all units in the Property as of the first day of the month. Resident represents that all occupants that will reside in Resident’s unit are identified in this Agreement. Resident agrees to immediately notify Landlord at any change in the number of occupants. `,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      ...
      {
        type:'text',
        val:`Factored occupancy method: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`Resident’s share is calculated using a formula that compares the “factored occupancy” figure for the number of authorized occupants residing in Resident’s unit as compared to the total “factored occupancy” figure for all authorized occupants in all units in the Property as of the first day of the month.  Factored occupancy is based on the premises that multiple people in a single unit generally use less utilities than the same number of single people living in separate units.  It applies a ratio to the number of occupants as follows:`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`               1 person = Factored occupancy of 1`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`               2 people = Factored occupancy of 1.6`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`               3 people = Factored occupancy of 1.9`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`               4 people = Factored occupancy of 2.2`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`               5 people = Factored occupancy of 2.5`,
        opt:p1Options
      },
      {type:'linebreak'},  
    ],
    ...
    [
      {
        type:'text',
        val:`Resident represents that all occupants that will reside in Resident’s unit are identified in this Agreement. Resident agrees to immediately notify Landlord at any change in the number of Occupants.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
      ...
      {
        type:'text',
        val:`Dividing the bill equally among all occupied units in the Property: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`Resident’s share is   __ % of the bill if all units are occupied.`,
        opt: p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`Landlord and Resident agree that it is impractical or extremely difficult to determine the exact amount of the utilities consumed by Resident (and/or in the common areas), but that the methods used to determine Resident’s share described above are reasonably accurate estimates.  However, if during the tenancy Landlord believes that Resident’s utility consumption is disproportionate to the charge to Resident as specified above, Landlord reserves the right to modify the method used to charge Resident, in Landlord’s reasonable discretion.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`Trash/Recycling Charges.`,
        opt:p1Bold
      },
      {
        type: 'text',
        val:`Trash/recycling charges include the cost of Landlord or Property Management employees to clean trash and recycling chutes, bins, containers, and trash areas, collect trash/recycling bins from enclosures, compact and sort the trash and recycling, and return the bins to the trash/recycling enclosures.`,
        opt:p1Options
      },
    ],
    [
      ...
      {
        type:'text',
        val:` Utility Billing Service:`,
        opt:p1Bold
      },
      {
        type:'text',
        val:`Landlord currently uses the services of          (“Utility Billing Company”) to bill for        [list Utilities.]  Landlord reserves the right to change utility billing service providers at any time.  If Landlord changes the utility billing service during Resident’s tenancy, Resident will be notified.  The Utility Billing Company is not the utility provider.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      ...
      {
        type:'text',
        val:` Landlord Billing: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`Landlord bills utilities to Resident.   The payment instructions on page 1 apply to utility billings as well as to any other payments due from Resident to Landlord, except as modified below.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`Utility Billing Statements: `,
        opt:p1Bold        
      },
      {
        type:'text',
        val:`Resident will receive monthly billing statements. Residents will generally receive bills on approximately the       day of the month (i.e. utilities consumed in January will be billing on approximately      ). Billing statements are payable within    days of the statement date.   The due date for Base Rent and utility charges may not coincide.  Resident’s failure to pay the utility charges by their due date will be considered a material breach of the Agreement, grounds for termination of the tenancy, and if allowed by law, we may discontinue providing the utilities to you.   Utility billings will be prorated as necessary. `,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      ...
      {
        type:'text',
        val:`Resident’s Meter Includes Utilities Consumed in Common Areas: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`Resident’s  gas  electric  water meter or submeter measures utilities consumed in both the Residence and in the following common area:      . Resident will be responsible for these utilities.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:` ${checkBox} Each bill will include a monthly service fee of $     `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`This monthly administrative fee is the actual third party costs of reading submeters and providing billing services incurred by Landlord. Resident must make payment in full to Landlord or the Utility Billing Company of the utility charges before the due date listed on each bill.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`Utility Late Fees: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`Landlord and Resident agree that the actual cost to Landlord and/or the Utility Billing Company when Resident fails to pay the utility bill on time is difficult or impossible to ascertain, but the parties agree that Landlord and/or the Utility Billing Company will incur certain costs, such as additional bookkeeping and administrative charges, additional charges, costs in printing and mailing late notices, and lost opportunity costs of the payment.  Accordingly, Landlord and Resident agree that if the utility payment is not paid by the due date, then Resident will incur a utility late fee of $     .`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`Resident Questions and Concerns Regarding Utility Billings: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`Resident concerns and questions regarding utility billings may be addressed to HomeTeam Property Management, 821 Bowsprit Road, Chula Vista, CA 91914 during the hours of 9:00 AM and 5:00 PM Monday through Friday.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      ...
      {
        type:'text',
        val:` ${checkBox} may  ${checkBox} may not be washed on the Property.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`Oil Changing and automobile repairs are not allowed on the property.`,
        opt: p1Bold
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`BAR-B-QUE GRILLS `,
        opt:p1Bold
      },
      {
        type:'text',
        val:` ${checkBox} are  ${checkBox} are not allowed.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`Renters Insurance`,
        opt:p1Bold
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`You are required to obtain and maintain Renter’s Insurance with minimum liability of $100,000.00 per occurrence and a maximum deductible of $1,000.00`,
        opt:p1Bold
      },
    ],
      [
      ...
      {
        type:'text',
        val:`Unless otherwise specified below, you may not possess, plant cultivate, harvest transport dry or process, marijuana or cannabis products in the residence or property.`,
        opt:p1Bold
      },
      {type:'linebreak'},
      {type:'linebreak'},
      {
        type:'text',
        val:`DESIGNATED SMOKE FREE AREAS: `,
        opt:p1Bold
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`             ${bulPoint}     The Residence`,
        opt:p1Bold
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`             ${bulPoint}     All Common Areas`,
        opt:p1Bold
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`             ${bulPoint}     The Residence Balcony/Patio`,
        opt:p1Bold
      },
    ],
    {
    ...
    [
      {
        type:'text',
        val:`ABESTOS DISCLOSURES APPLY: If indicated, the residence was built before 1981 when asbestos was still used in construction, and the Asbestos section of this agreement will apply.`,
        opt:p1Bold
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`Landlord knowledge of asbestos hazards in the Residence or Property:`,
        opt:p1Bold
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`${checkBox} (If checked) Landlord has no knowledge of any asbestos hazards in the Residence or Property, but because of the age of the Property, Resident should review the asbestos section of this Agreement.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`${checkBox} (If checked)  Landlord is aware of the following asbestos hazards in the Residence or Property:      `,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`Reports or records pertaining to asbestos hazards in the Residence or Property:`,
        opt:p1Bold
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`${checkBox} (If checked)  Landlord is not aware of any reports or records pertaining to asbestos hazards in the Residence or Property.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`${checkBox} (If checked)  Available reports or records pertaining to asbestos hazards in the Residence or Property are as follows:        `,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`Copies of the reports or records identified are available for Resident’s review at      . `,
        opt:p1Options
      }
    ],
    {
      type:'text',
      val:`WARNING: Asbestos is a chemical known to the state of California to cause cancer. For more information go to www.P65Warnings.ca.gov.`,
      opt:p1Bold
    },
    {
      type:'text',
      val:`Resident may obtain information about hazards, including flood hazards, that may affect the Property from the Office of Emergency Services at http://myhazards.caloes.ca.gov/. Landlord’s owner’s insurance does not cover the loss of Resident’s personal possessions and it is recommended that Resident consider purchasing renter’s insurance and flood insurance to insure Resident’s possessions from loss due to fire, flood, or other risk of loss.`,
      opt:p1Options
    },
    [
      {
      ...
      {
        type:'text',
        val:` FLOOD DISCLOSURES APPLY: `,
        opt:p1Bold
      },
      {
        type:'text',
        val:`If indicated, the Residence is located in a special flood hazard area or an area of potential flooding.  Landlord is not required to provide additional information concerning the flood hazards to the property and the information provided in this section is deemed adequate to inform Resident.`,
        opt:p1Options
      },
    ],
    [
    ...
    {
      type:'text',
      val:`PROPOSITION 65 Warning: You may be exposed at the property to chemicals known to the State of California to cause cancer, birth defects, and other reproductive harm. These chemicals may be contained in emissions and fumes from building materials (such as insulation,  pressed wood materials, finishes,  adhesives, paint chips and dust, plumbing materials, and imported vinyl miniblinds manufactured before 1997), products and materials used to maintain the property, and emissions, fumes, and smoke from Resident and guest activities, including but not limited to the use of motor vehicles, enclosed parking facilities, barbecues, fireplaces, unvented gas space heaters,  and smoking. `,
      opt:p1Bold
    },
    {
      type:'text',
      val:`These chemicals may include, but are not limited to asbestos, lead, carbon monoxide, formaldehyde, tobacco smoke,   nicotine,  gasoline and diesel engine exhaust. Talk to your landlord or the building owner about how and when you could be exposed to these chemicals in your building. For additional information go to www.p65warnings.ca.gov/apartments. For more information go to www.p65warnings.ca.gov, www.p65warnings.ca.gov/places/enclosed-parking-facilities (if the property has an enclosed garage) and www.p65warnings.ca.gov/places/designated-smoking-areas (if smoking occurs on the property).`,
      opt:p1Bold
    },
    [
      {
      ...
      {
        type:'text',
        val:` ${checkBox} (If checked)  Pesticides are periodically applied to  ${checkBox} the Residence ${checkBox} units near the Residence ${checkBox} common areas. They are applied by  a registered structural pest control company ${checkBox} Landlord or Landlord’s agents.  `,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`The pesticides are meant to control the following pest(s):     .  The approximate date, time and frequency of the pesticide treatment is      . The approximate date, time and frequency of the pesticide application is subject to change.  The pesticide(s) name, brand and active ingredient:       or ${checkBox} as specified in a separate pesticide notice.  `,
        opt:p1Bold
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`State law requires that you be given the following information. CAUTION--PESTICIDES ARE TOXIC CHEMICALS.  `,
        opt:p1Bold
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`Structural Pest Control Companies are registered and regulated by the Structural Pest Control Board, and apply pesticides which are registered and approved for use by the California Department of Pesticide Regulation and the United States Environmental Protection Agency. Registration is granted when the state finds that based on existing scientific evidence there are no appreciable risks if proper use conditions are followed or that the risks are outweighed by the benefits.  `,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`The California Department of Pesticide Regulation and the United States Environmental Protection Agency allow the unlicensed use of certain pesticides based on existing scientific evidence that there are no appreciable risks if proper use conditions are followed or that the risks are outweighed by the benefits.  `,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`The degree of risk depends upon the degree of exposure, so exposure should be minimized.  `,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`If within 24 hours following application, you experience symptoms similar to common seasonal illness comparable to the flu, immediately contact  `,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`${bulPoint}     your physician or the California Poison Control System (1-800-222-1222), and`,
        opt:p1Options
      },
      {
        type:'text',
        val:`${bulPoint}     if the pesticide was applied by a pest control company, also contact the pest control company.`,
        opt:p1Options
      },
      {
        type:'text',
        val:`(Modify this statement, based on the pesticides applied, to include any other symptoms of overexposure, which are not typical of influenza.)`,
        opt:p1Options
      },
      {
        type:'text',
        val:`For further information, contact any of the following: `,
        opt:p1Options
      },
      {
        type:'text',
        val:`${bulPoint}      (if pesticide is applied by a pest control company) the pest control company, telephone number:       or  ${checkBox} as specified in a separate pesticide notice`,
        opt:p1Options
      },
      {
        type:'text',
        val:`${bulPoint}      for Health Questions--the County Health Department, telephone number:       or ${checkBox} as specified in a separate pesticide notice`,
        opt:p1Options
      },
      {
        type:'text',
        val:`${bulPoint}       for Application Information--the County Agricultural Commissioner, telephone number:        or ${checkBox} as specified in a separate pesticide notice `,
        opt:p1Options
      },
      {
        type:'text',
        val:`${bulPoint}       for Regulatory Information:`,
        opt:p1Options
      },
      {
        type:'text',
        val:`${bulPoint}  the Structural Pest Control Board, 2005 Evergreen St # 1500, Sacramento, CA 95815 (916) 561-8708 (if pesticide is applied by a pest control company)`,
        opt:p1Options
      },
      {
        type:'text',
        val:`the Department of Pesticide Regulation (916-324-4100) (if the pesticide is not applied by a pest control company)`,
        opt:p1Options
      },
    ],
    [
      ...
      {
        type:'text',
        val:' ONGOING CONSTRUCTION: ',
        opt:p1Bold
      },
      {
        type:'text',
        val:`If indicated, there is ongoing construction in the Property consisting of:      . The estimated date of completion of construction is      . Construction will normally be limited to the following days of the week:       and the following hours:        to      . `,
        opt:p1Options
      },
      {type:'linebreak'},
      {
      ...
      {
        type:'text',
        val:` Information provided to Resident regarding the Property may refer to amenities for which construction may not yet be completed including the following:       .`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
      ...
      {
        type:'text',
        val:' CONDOMINIUM CONVERSION: ',
        opt:p1Bold
      },
      {
        type:'text',
        val:`The Residence has been approved for sale to the public as a condominium project.  You may be given additional documents about this.`,
        opt:p1Options
      },
      {type:'linebreak'},
      {
        type:'text',
        val:`Additional notices, disclosures and terms:         `,
        opt:p1Options
      }
    ],
    {type:'linebreak'},
    ...
    {
      type:'text',
      val:`INITIAL AMOUNTS DUE: `,
      opt:p1Bold
    },
    {
      type:'text',
      val:`The following initial amounts are due under this Agreement as specified: `,
      opt:p1Bold
    },
    {
      type:"table",
      val: initialAmountsDue,
      opt: tableStyle
    },
      
      {
        type:'text',
        val:`The payments described above must be made by:    ${checkBox} Money Order   ${checkBox}  Cashiers Check    ${checkBox}  Cash   ${checkBox}  Personal Check       `,
        opt:p1Options
      },
      {
        type:'text',
        val:'AGREEMENT ADDENDA AND OTHER  WRITTEN MATERIALS PROVIDED TO RESIDENT:',
        opt:p1Bold
      },
      
      {
        type:'text',
        val:` ${checkBox} Pesticide Notice`,
        opt:p1Options
      },
      
      {
        type:'text',
        val:` ${checkBox} Asbestos records and reports`,
        opt:p1Options
      },
      ...
      {
        type:'text',
        val:`              ${checkBox} Rules and Regulations`,
        opt:p1Options
      },
      ...
      {
        type:'text',
        val:'Created on       by Leasing Agent:       ',
        opt:p1Options
      },
      [
    ...
    {
      type:'text',
      val:`         ${bulPoint} Clean the Residence at the termination of the tenancy, if not returned to us at the same level of cleanliness as received;`,
      opt:p1Options
    },
    {
      type:'text',
...
var minute = today2.getMinutes();
today2 = mm + dd + yyyy + hour + minute;

  let docPath = `${today2}-${information[0].streetAddress}`

  let out = fs.createWriteStream(`./LeaseAgreement/${docPath}.docx`);
